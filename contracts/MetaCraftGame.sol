/**
 *Submitted for verification at hecoinfo.com on 2021-09-22
*/
...
contract MetaCraft is ERC721Enumerable,IMetaCraft {
    ...
    string constant public name = "MetaCraft";
    string constant public symbol = "MCT";
    ...
    function set_mint_fee(uint _fee) external {
        require(msg.sender == owner,'not owner call');
        mint_fee = _fee;
    }
    ...
    function get_all_summoners(address _owner) view external returns(uint[] memory) {
        ...
        for(uint i = 0; i < count; i++) {
            uint _summerId = tokenOfOwnerByIndex(_owner,i);
            res[i] = _summerId;
        }
        return res;
    }
    ...
    function tokenURI(uint256 _summoner) public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "summoner #', toString(_summoner), '", "description": "Metacraft is achieved via an active economy, summoners must level, gain feats, learn spells, to be able to craft gear. This allows for market driven Metacraft while allowing an ever growing economy. Feats, spells, and summoner gear is ommitted as part of further expansions.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
